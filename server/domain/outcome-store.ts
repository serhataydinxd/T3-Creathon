 ...
 *
 * "Outcome" here means an official MEB learning outcome and nothing else.
 * Workshop topics — including every published Bilim Türkiye catalogue entry —
 * live in `topics` and are handled by topic-store.ts. Rows this table still
 * holds from before that split are marked `legacy_catalogue_topic` and are
 * never treated as outcomes.
 */
