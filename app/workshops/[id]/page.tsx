      ...
      <section className="page persisted-workshop">
        ...
        <div className="persisted-grid">
          <div className="package-stages">
            {plan.stages.map((stage, index) => <article key={stage.key}><div className="package-stage-title"><span>0{index + 1}</span><div><small>{stage.name} · {stage.minutes} dk</small><h2>{stage.title}</h2></div></div><div className="package-columns"><p><b>Eğitmen</b>{stage.teacherAction}</p><p><b>Öğrenci</b>{stage.studentAction}</p></div><div className="trace-line"><LockKeyhole /><div><strong>Kazanım bağlantısı</strong><p>{stage.objectiveConnection}</p></div></div><div className="evidence-box"><ShieldCheck /><div><strong>Öğrenme kanıtı</strong><p>{stage.evidence}</p></div></div></article>)}
          </div>
