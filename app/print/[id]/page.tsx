...
export default async function PrintPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <main className="print-document">
      ...
      {plan.stages.map((stage, index) => <article key={stage.key}><span>0{index + 1} · {stage.name} · {stage.minutes} dk</span><h2>{stage.title}</h2><p><b>Eğitmen:</b> {stage.teacherAction}</p><p><b>Öğrenci:</b> {stage.studentAction}</p><p><b>Kazanımla bağlantı:</b> {stage.objectiveConnection}</p><p><b>Kanıt:</b> {stage.evidence}</p></article>)}
